...
def load_svg(fname, rate=5):
    ...
    for path in paths[ind]:
        ...
        if isinstance(path, Line):
            ...
            alpha = np.linspace(
                0, 1, int(np.ceil(length) / (3 * rate)), endpoint=False)
            if len(alpha):
...
def transform(f, threshold=0.9):
    ...
    # Save the calculated values to a .mat file
    savemat('fourier_data.mat', {
        'N': N,
        'F_0': F[0],
        'F_sel': F_sel,
        'M_sel': M_sel,
...
def plot_circle(c, r, t, r_i, N=101, colour=None):
    ...
    pl.plot([c.real, c.real + r_i.real],
            [c.imag, c.imag + r_i.imag], c=colour, lw=0.5)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Approximate complex functions with Fourier decomposition.')
    parser.add_argument('--precision', dest='precision', default=0.99,
                        help='The percentage precision in estimating the Fourier transform. In range (0-1). Default 0.99.', type=float)
    ...
    parser.add_argument('--rate', dest='rate', default=2,
                        help='Downsampling rate for the input image. Higher values give coarser approximation but will likely be faster to complete. Default=2.', type=float)
    parser.add_argument(
        'svg', help='The name of the SVG file to be processed.')
    parser.print_help()
